...
public class Main {
    public static void main(String[] args) throws Exception {
        if (args.length != 4) {
            System.err.println("4 arguments needed: <input_path> <output_path> <num_reducers> <block_size (kb)");
            System.exit(-1);
        }
        ...
        int datablockSize = Integer.parseInt(args[3]) * 1024;
        ...
        if (fs.exists(intermediateOutput)) {
            log.info("Removing intermediate existing directory: {}", intermediateResultDir);
            fs.delete(intermediateOutput, true);
        }

        if (fs.exists(finalOutput)) {
            log.info("Removing final existing directory: {}", outputDir);
            fs.delete(finalOutput, true);
        }

        conf.set("mapreduce.input.fileinputformat.split.maxsize", Integer.toString(datablockSize));
        ...
        Job sortByValueJob = Job.getInstance(conf, "sorting by price");
